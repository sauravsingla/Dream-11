...
def prepare_player_data(
    ...
    # ddof=0 reproduces numpy.std used in the original notebook.
...
def optimise_team(
    ...
    Returns the selected rows with ``Fantasy Role``, ``Score Multiplier`` and
    ``Risk-adjusted Contribution`` columns.
    """

    if gp is None or GRB is None:
        raise RuntimeError(
            "gurobipy is required to solve the model. Install it and configure "
            "a valid Gurobi licence before calling optimise_team()."
        )
    ...
    player_indices = list(prepared.index)

    model = gp.Model("dream11_team_selection")
    model.Params.OutputFlag = 0
    selected = model.addVars(roles, player_indices, vtype=GRB.BINARY, name="selected")

    expected = prepared["Expected Score"].to_dict()
    risk = prepared["Score Risk"].to_dict()
    price = prepared["Price"].to_dict()

    model.setObjective(
        gp.quicksum(
            (
                multipliers[role] * expected[index]
                - config.risk_aversion * risk[index]
            )
            * selected[role, index]
            for role in roles
    ...
    for index in player_indices:
        model.addConstr(
            gp.quicksum(selected[role, index] for role in roles) <= 1,
            name=f"one_role_{index}",
        )

    for role in roles:
        model.addConstr(
            gp.quicksum(selected[role, index] for index in player_indices)
            == role_counts[role],
            name=f"role_count_{role.lower()}",
        )

    model.addConstr(
        gp.quicksum(
            price[index] * selected[role, index]
            for role in roles
            for index in player_indices
        )
        <= config.budget,
    ...
    for player_type, (minimum, maximum) in config.role_limits.items():
        ...
        model.addConstr(count >= minimum, name=f"min_{player_type.lower()}")
        model.addConstr(count <= maximum, name=f"max_{player_type.lower()}")

    for team_name, group in prepared.groupby("Team"):
        count = gp.quicksum(
            selected[role, index]
            for role in roles
        ...
        model.addConstr(
            count <= config.max_players_per_team,
            name=f"team_limit_{str(team_name).replace(' ', '_')}",
        )
